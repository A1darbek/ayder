...
RUN_ID="provider_timeout_$(date -u +%Y%m%dT%H%M%SZ)"
ARTIFACT_DIR="${ARTIFACT_DIR:-${REPO_ROOT}/artifacts/provider_timeout/${RUN_ID}}"
...
validate_receipt() {
  ...
  [[ "$(jq -r '.financial_determinism' "$receipt")" == "FAIL" ]] || return 1
  [[ "$(jq -r '.summary.payment_attempts' "$receipt")" == "5" ]] || return 1
  [[ "$(jq -r '.summary.confirmed_success' "$receipt")" == "2" ]] || return 1
  [[ "$(jq -r '.summary.confirmed_failed' "$receipt")" == "1" ]] || return 1
  [[ "$(jq -r '.summary.ambiguous' "$receipt")" == "1" ]] || return 1
  [[ "$(jq -r '.summary.suppressed_duplicates' "$receipt")" == "1" ]] || return 1
  [[ "$(jq -r '.summary.duplicate_charges' "$receipt")" == "0" ]] || return 1
  [[ "$(jq -r '.summary.financially_indeterminate_attempts' "$receipt")" == "1" ]] || return 1
  [[ "$(jq -r '.indeterminate_attempts[0].attempt_id' "$receipt")" == "pay_attempt_004" ]] || return 1
}

log "provider-timeout demo run_id=${RUN_ID} artifact_dir=${ARTIFACT_DIR}"
...
"${SCRIPT_DIR}/replay_consumer.sh" >"${ARTIFACT_DIR}/replay_consumer.log" 2>&1
...
log "provider-timeout demo success"
