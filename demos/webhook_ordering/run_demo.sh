...
RUN_ID="webhook_ordering_$(date -u +%Y%m%dT%H%M%SZ)"
ARTIFACT_DIR="${ARTIFACT_DIR:-${REPO_ROOT}/artifacts/webhook_ordering/${RUN_ID}}"
...
validate_receipt() {
  ...
  [[ "$(jq -r '.payment_id' "$receipt")" == "pay_123" ]] || return 1
  [[ "$(jq -r '.final_state' "$receipt")" == "SUCCESS" ]] || return 1
  [[ "$(jq -r '.suppressed_attempts | length' "$receipt")" == "2" ]] || return 1
  [[ "$(jq -r '.suppressed_attempts[] | select(.event_id=="webhook_002").reason' "$receipt")" == "stale_state_transition" ]] || return 1
  [[ "$(jq -r '.suppressed_attempts[] | select(.event_id=="webhook_003").reason' "$receipt")" == "duplicate_webhook" ]] || return 1
  [[ "$(jq -r '.business_rules[] | select(.status!="PASS") | .id' "$receipt" | wc -l | tr -d ' ')" == "0" ]] || return 1
}

log "webhook-ordering demo run_id=${RUN_ID} artifact_dir=${ARTIFACT_DIR}"
...
"${SCRIPT_DIR}/webhook_consumer.sh" >"${ARTIFACT_DIR}/webhook_consumer.log" 2>&1
...
log "webhook-ordering demo success"
