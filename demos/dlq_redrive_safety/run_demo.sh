...
RUN_ID="dlq_redrive_safety_$(date -u +%Y%m%dT%H%M%SZ)"
ARTIFACT_DIR="${ARTIFACT_DIR:-${REPO_ROOT}/artifacts/dlq_redrive_safety/${RUN_ID}}"
...
validate_receipt() {
  ...
  [[ "$(jq -r '.dlq_summary.items' "$receipt")" == "1" ]] || return 1
  [[ "$(jq -r '.dlq_summary.safe_to_redrive' "$receipt")" == "0" ]] || return 1
  [[ "$(jq -r '.dlq_summary.manual_review_required' "$receipt")" == "1" ]] || return 1
  [[ "$(jq -r '.dlq_items[0].message_id' "$receipt")" == "msg_009" ]] || return 1
  [[ "$(jq -r '.dlq_items[0].replay_safety' "$receipt")" == "UNKNOWN" ]] || return 1
}

log "dlq-redrive-safety demo run_id=${RUN_ID} artifact_dir=${ARTIFACT_DIR}"
...
"${SCRIPT_DIR}/dlq_consumer.sh" >"${ARTIFACT_DIR}/dlq_consumer.log" 2>&1
...
log "dlq-redrive-safety demo success"
