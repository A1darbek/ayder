...
static inline void rf_invalidate_slot(rf_partition_t *p, rf_msg_slot_t *slot) {
  ...
  if (b)
    atomic_fetch_sub_explicit(&p->live_bytes, b, memory_order_relaxed);
}
...
static inline void rf_size_sweep_if_over(rf_partition_t *p, uint64_t now) {
  ...
  if (!maxb)
    return;
  if (atomic_load_explicit(&p->live_bytes, memory_order_acquire) <= maxb)
    return;

  uint64_t last = atomic_load_explicit(&p->last_size_gc_us, memory_order_acquire);
  if (now - last < 2000)
    return; /* ~2ms throttle like TTL */
  if (!atomic_compare_exchange_strong(&p->last_size_gc_us, &last, now))
    return;
  ...
  for (uint64_t i = 0; i < STEPS && o < wh; i++, o++) {
    ...
    if (so != o)
      continue; /* already overwritten / not the intended logical slot */
...
static uint32_t reserve_heap(size_t sz) {
  ...
  for (;;) {
    ...
    if (atomic_compare_exchange_weak_explicit(&g_bus->data_head, &cur, cur + adv, memory_order_acq_rel,
...
static rf_msg_slot_t *rf_ring_alloc(size_t cap) {
  size_t bytes;
  if (__builtin_mul_overflow(cap, sizeof(rf_msg_slot_t), &bytes))
    return NULL;
...
static int rf_partition_ensure_ring(rf_partition_t *p) {
  ...
  if (!atomic_compare_exchange_strong_explicit(&p->ring, &expected, newr, memory_order_release, memory_order_relaxed)) {
...
int rf_bus_init_pre_fork(size_t heap_bytes, size_t desc_count) {
  ...
  void *mem = mmap(NULL, total, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
  if (mem == MAP_FAILED)
    return -1;
...
int ha_fwd_send(uint32_t type, const void *payload, uint32_t size) {
  ...
  h->size         = size;
  if (size)
    memcpy((uint8_t *)buf + sizeof(*h), payload, size);
...
void rf_bus_start_promoter(void) {
  ...
  int expect                 = 0;
  if (!atomic_compare_exchange_strong(&started, &expect, 1))
    return;
...
static inline int simd_strcmp(const char *a, const char *b, size_t len) {
  ...
  if (len >= 32) {
    ...
    if (rem)
      return memcmp(a + (len - rem), b + (len - rem), rem);
    return 0;
  }
#endif
...
static inline uint64_t fetch_add_u64(_Atomic uint64_t *obj, uint64_t inc, memory_order mo) {
  ...
  for (;;) {
    ...
    if (atomic_compare_exchange_weak_explicit(obj, &cur, next, mo, memory_order_relaxed)) {
      return cur;
    }
...
static int idemp_get(const char *topic, int part, const void *unused_key, size_t unused_len, uint64_t fp64_in,
                     uint64_t *out_offset) {
  (void)unused_key;
  (void)unused_len;
  if (!g_shared_storage)
  ...
  aof_idemp_val_t v;
  if (!shared_storage_get_fast(g_shared_storage, skey, &v, sizeof v))
    return 0;
  if (v.fp64 != fp64_in)
    return 0; // guard against 32-bit collision
  if (out_offset)
    *out_offset = v.offset;
  return 1;
}

static int idemp_put(const char *topic, int part, const void *unused_key, size_t unused_len, uint64_t fp64_in,
                     uint64_t offset) {
  (void)unused_key;
  (void)unused_len;
  if (!g_shared_storage)
...
uint32_t rf_topic_id(const char *name) { return simd_hash_djb2(name, strlen(name)); }
...
static rf_topic_t *find_topic_lockfree(const char *name, uint32_t id) {
  ...
  for (int probe = 0; probe < 8; probe++) {
    ...
    if (t && t->id == id) {
      ...
      if (tlen == nlen && simd_strcmp(t->name, name, nlen) == 0)
        return t;
    }
    if (!t)
      break;
...
static inline void atomic_max_u64(_Atomic uint64_t *dst, uint64_t val) {
  ...
  while (cur < val &&
         !atomic_compare_exchange_weak_explicit(dst, &cur, val, memory_order_acq_rel, memory_order_acquire)) {
    /* keep trying */
  }
}
...
static inline void rf_ttl_sweep_if_due(rf_partition_t *p, uint64_t now_mono, uint64_t cutoff_wall) {
  ...
  if (now_mono - last < 2000)
    return; /* throttle ~2ms; adjust as you like */
  if (!atomic_compare_exchange_strong(&p->last_ttl_gc_us, &last, now_mono))
    return;
...
int rf_set_max_bytes(const char *topic, int part, uint64_t max_bytes) {
  ...
  for (int i = p0; i < p1; i++)
    rf_size_apply_to_partition(&t->parts[i], max_bytes);
  return 0;
}
...
int rf_topic_ensure(const char *name, int partitions) {
  ...
  for (int i = 0; i < slot->partitions; i++) {
    if (init_partition_lockfree(&slot->parts[i]) != 0)
      return -4;
  }
  ...
  for (int probe = 0; probe < 8; probe++) {
    ...
    if (atomic_compare_exchange_weak_explicit(&g_topic_hash[idx_hash], &expected, slot, memory_order_release,
                                              memory_order_relaxed)) {
      break;
    }
  }
...
int rf_delete_before(const char *topic, int partition, uint64_t before_offset, uint64_t *deleted_count,
                     uint64_t *freed_bytes) {
  if (!topic)
    return -1;

  rf_topic_t *t = find_topic_lockfree(topic, rf_topic_id(topic));
  if (!t || partition < 0 || partition >= t->partitions)
    return -2;
  ...
  if (ring && limit > oldest) {
    for (uint64_t o = oldest; o < limit; o++) {
      ...
      uint64_t so = atomic_load_explicit(&s->offset, memory_order_acquire);
      if (so != o)
        continue; /* wrong logical occupant */
  ...
  aof_truncate_rec_t rec = {
      .topic_id = t->id, .partition = (uint16_t)partition, .reserved = 0, .before_offset = before_offset};
...
static inline void replay_one_msg_rec(const aof_msg_rec_t *r, const uint8_t *kv) {
  ...
  if (rf_partition_ensure_ring(p) != 0)
    return;
  rf_msg_slot_t *ring = atomic_load_explicit(&p->ring, memory_order_acquire);
  if (!ring)
    return;
  ...
  if (r->offset < floor)
    return; // NEW: honor truncate floor
...
int rf_produce_sealed(const char *topic, int partition, const void *key, size_t key_len, const void *val,
                      size_t val_len, uint64_t *out_offset, uint64_t *batch_id) {
  if (!topic || (!val && val_len))
    return -1;
  ...
  if (total_size <= sizeof(slot->inline_data)) {
    ...
    if (!blob)
      return -4;
    if (key && key_len)
      memcpy(blob, key, key_len);
    if (val && val_len)
      memcpy((uint8_t *)blob + key_len, val, val_len);
    slot->blob = blob;
  }
...
size_t rf_consume(const char *topic, const char *group, int partition, uint64_t from_exclusive, size_t max_msgs,
  ...
  if (current_offset < min_available)
    current_offset = min_available;

  /* NEW: skip any offsets below the delete floor */
  uint64_t floor = atomic_load_explicit(&p->delete_floor, memory_order_acquire);
  if (current_offset < floor)
  ...
  while (produced < cap && scanned < avail) {
    ...
    if ((flags & 1) && so == current_offset) {
      ...
      out[produced] = (rf_msg_view_t){.key       = slot->blob,
                                      .key_len   = klen,
...
If it’s still being published, skipping would drop it → stop here. */
  ...
  if (next_offset_out)
    *next_offset_out = current_offset;
  return produced;
}
// ═══════════════════════════════════════════════════════════════════════════════
...
void rf_broker_replay_aof(uint32_t rec_id, const void *data, size_t sz) {
  ...
  if (rec_id == AOF_REC_BROKER_TRUNCATE) {
    if (sz < sizeof(aof_truncate_rec_t))
      return;
  ...
  if (rec_id == AOF_REC_BROKER_TTL) {
    if (sz < offsetof(aof_ttl_rec_t, topic))
      return;
    aof_ttl_rec_t r = {0};
    size_t copy     = sz;
    if (copy > sizeof(r))
    ...
    if (r.topic[0] == 0 || (r.topic[0] == '*' && r.topic[1] == 0)) {
      if (ttl_ms)
        rf_set_ttl_ms_all(ttl_ms);
      rf_set_max_bytes_all(maxb);
    } else {
      if (ttl_ms)
        rf_set_ttl_ms(r.topic, r.partition, ttl_ms);
  ...
  if (rec_id == AOF_REC_BROKER_MSG_IDEMP) {
    if (sz < sizeof(aof_msg_rec_t) + sizeof(uint64_t))
      return;
    const aof_msg_rec_t *r = (const aof_msg_rec_t *)data;
    const uint8_t *kv      = (const uint8_t *)data + sizeof(*r);
    size_t tot             = r->key_len + r->val_len;
    if (sizeof(*r) + tot + sizeof(uint64_t) > sz)
      return;
    uint64_t fp64;
  ...
  if (rec_id == AOF_REC_BROKER_MSG_BATCH_IDEMP) {
    ...
    int n                      = atomic_load_explicit(&g_topic_count, memory_order_acquire);
    for (int i = 0; i < n; i++)
      if (g_topics[i].id == first->topic_id) {
        t = &g_topics[i];
        break;
      }
    if (t) {
...
int rf_produce_batch_sealed(const char *topic, int partition, const rf_msg_view_t *messages, size_t count,
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (total_size <= sizeof(slot->inline_data)) {
      ...
      if (msg->key && msg->key_len)
        memcpy(blob, msg->key, msg->key_len);
      if (msg->val && msg->val_len)
        memcpy((uint8_t *)blob + msg->key_len, msg->val, msg->val_len);
      slot->blob = blob;
    }
  ...
  if (rocket && atoi(rocket) != 0) {
    ...
    if (batch_id)
      *batch_id = 0; /* no durable batch id */
  } else
#endif
  {
...
int rf_produce_sealed_idemp(const char *topic, int partition, const void *key, size_t key_len, const void *val,
                            size_t val_len, const void *idemp_key, size_t idemp_key_len, uint64_t *out_offset,
  ...
  *is_duplicate = 0;
  ...
  if (total <= sizeof(slot->inline_data))
    memcpy(w, slot->inline_data, total);
  else
...
int rf_produce_batch_sealed_idemp(const char *topic, int partition, const rf_msg_view_t *messages, size_t count,
  ...
  *is_duplicate = 0;
  ...
  if (first_offset)
    *first_offset = off0;
  uint64_t ts_wall = wall_us_raw();
  /* Compute sealed payload size: for each msg: [aof_msg_rec_t][kv] and after the last msg, append fp64 once */ size_t
      total_aof = sizeof(uint64_t);
  /* fp64 trailer */ for (size_t i = 0; i < count; i++)
    total_aof += sizeof(aof_msg_rec_t) + messages[i].key_len + messages[i].val_len;
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (tot <= sizeof(slot->inline_data)) {
      if (m->key && m->key_len)
        memcpy(slot->inline_data, m->key, m->key_len);
      if (m->val && m->val_len)
        memcpy(slot->inline_data + m->key_len, m->val, m->val_len);
      slot->blob = slot->inline_data;
    } else {
      ...
      if (m->key && m->key_len)
        memcpy(blob, m->key, m->key_len);
      if (m->val && m->val_len)
        memcpy((uint8_t *)blob + m->key_len, m->val, m->val_len);
      slot->blob = blob;
    }
  ...
  idemp_put(topic, partition, idemp_key, idemp_key_len, fp64, off0);
  return 0;
}
