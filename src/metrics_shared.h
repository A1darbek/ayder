...
{
    size_t sz = (sizeof(RAMForgeMetrics) + getpagesize() - 1) &
                ~(getpagesize() - 1);            /* page-align */

    int fd = shm_open(RAMFORGE_METRICS_SHM,
                      O_CREAT | O_RDWR, 0666);
    if (fd < 0) {
...
static inline void metrics_cleanup_shared(void)
{
    shm_unlink(RAMFORGE_METRICS_SHM);
}
