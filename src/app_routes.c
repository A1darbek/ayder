...
static inline size_t jout_avail(const jout_t *o) { return (o->p < o->end) ? (size_t)(o->end - o->p) : 0; }
...
static int build_projected_object_json(char *out, size_t cap, json_value_t *obj, json_value_t *fields) {
  ...
  if (fields && fields->type == JSON_ARRAY && fields->as.array.count > 0) {
    for (size_t i = 0; i < fields->as.array.count; i++) {
      ...
      if (fn->type != JSON_STRING)
        continue;

      char name[64];
      if (!jv_to_cstr(fn, name, sizeof name))
        continue;
      ...
      case JSON_STRING:
        ...
        if (!jout_try_c(&w, '"'))
          return 0;
        if (!jout_try_mem(&w, fv->as.s.ptr, fv->as.s.len))
          return 0;
        if (!jout_try_c(&w, '"'))
          return 0;
        break;
      case JSON_INT:
        if (!jout_try_printf(&w, "%" PRId64, (int64_t)fv->as.i))
          return 0;
        break;
      case JSON_DOUBLE:
        if (!jout_try_printf(&w, "%.17g", fv->as.d))
          return 0;
        break;
      case JSON_BOOL:
        if (!jout_try_mem(&w, fv->as.b ? "true" : "false", fv->as.b ? 4 : 5))
          return 0;
        break;
      case JSON_NULL:
      default:
        if (!jout_try_mem(&w, "null", 4))
          return 0;
        break;
      }
    }
...
static void send_error(Response *res, const error_response_t *err) {
  int n = snprintf(res->buffer, RESPONSE_BUFFER_SIZE, "{\"ok\":false,\"error\":\"%s\",\"message\":\"%s\"%s%s%s}",
                   err->error, err->message, err->docs ? ",\"docs\":\"" : "", err->docs ? err->docs : "",
                   err->docs ? "\"" : "");
...
static int wait_synced_until(uint64_t batch_id, int timeout_ms) {
  if (batch_id == 0 || timeout_ms <= 0)
    return -1;
  uint64_t start = now_us_monotonic();
  for (;;) {
    if (AOF_sealed_last_synced_id() >= batch_id)
      return 1;
    if ((now_us_monotonic() - start) / 1000ULL >= (uint64_t)timeout_ms)
      return 0;
...
static inline int kv_int_key(const char *ns, const char *key) {
  ...
  h ^= fnv1a32(ns, strlen(ns));
  h *= 16777619u;
  h ^= (uint32_t)'/';
  h *= 16777619u;
  h ^= fnv1a32(key, strlen(key));
  h *= 16777619u;
...
static inline int jv_to_double(const json_value_t *v, double *out) {
  ...
  case JSON_STRING: {
    ...
    char buf[64];
    size_t n = v->as.s.len < sizeof(buf) - 1 ? v->as.s.len : sizeof(buf) - 1;
...
static int eval_condition(json_value_t *obj, const char *field, const char *op, json_value_t *want) {
  ...
  if (!(have_num && want_num))
    return 0; /* gt/ge/lt/le need numeric */
...
static int obj_matches_filters(json_value_t *obj, json_value_t *filters) {
  if (!filters || filters->type != JSON_ARRAY)
    return 1; /* no filters == pass */
  for (size_t i = 0; i < filters->as.array.count; i++) {
    ...
    if (!f || !o || !v || f->type != JSON_STRING || o->type != JSON_STRING)
      return 0;
    char fbuf[64], obuf[16];
    jv_to_cstr(f, fbuf, sizeof fbuf);
    jv_to_cstr(o, obuf, sizeof obuf);
    if (!eval_condition(obj, fbuf, obuf, v))
      return 0;
  }
  return 1;
}
...
static size_t build_group_key(json_value_t *obj, json_value_t *group_by, char *out, size_t cap) {
  size_t o = 0;
  if (!group_by || group_by->type != JSON_ARRAY)
    return 0;
  for (size_t i = 0; i < group_by->as.array.count; i++) {
    json_value_t *fld = &group_by->as.array.items[i];
    if (fld->type != JSON_STRING)
      continue;
    char name[64];
    jv_to_cstr(fld, name, sizeof name);
    ...
    char vb[256];
    vb[0] = 0;
    if (!jv_to_cstr(v, vb, sizeof vb))
      strncpy(vb, "null", sizeof vb);
    size_t l = strlen(vb);
    if (o + l >= cap)
      l = (cap > o ? cap - o - 1 : 0);
    if (l) {
...
int broker_query_handler(Request *req, Response *res) {
  ...
  if (!root || root->type != JSON_OBJECT) {
    ...
    memcpy(res->buffer, e, strlen(e));
    res->buffer[strlen(e)] = 0;
    if (root)
      json_free(root);
    return 0;
  }
  ...
  jv_to_cstr(jt, topic, sizeof topic);
  char group[RF_MAX_GROUP_LEN] = {0};
  if (jg && jg->type == JSON_STRING)
    jv_to_cstr(jg, group, sizeof group);

  int partition = (jp->type == JSON_INT) ? (int)jp->as.i : (int)jp->as.d;

  size_t limit = 200;
  if (jl && jl->type == JSON_INT && jl->as.i > 0 && jl->as.i <= 2000)
    limit = (size_t)jl->as.i;
  ...
  if (jo && (jo->type == JSON_INT || jo->type == JSON_DOUBLE)) {
    ...
    if (rf_offset_load(topic, group, partition, &committed))
      from_exclusive = committed;
  }
  ...
  jout_t out;
  jout_init_res(&out, res);

  /* reserve tail so we can always close JSON */
  const size_t TAIL_RESERVE = 512;
  char *hard_end            = out.end;
  if ((size_t)(hard_end - res->buffer) > TAIL_RESERVE)
    out.end = hard_end - (ptrdiff_t)TAIL_RESERVE;
  ...
  for (size_t i = 0; i < nread; i++) {
    ...
    if (!obj_matches_filters(obj, jflt)) {
      json_free(obj);
      continue;
    }
  ...
  for (size_t i = 0; i < agg_n; i++) {
    char rec[512];
    jout_t w;
    jout_init_buf(&w, rec, sizeof rec);
    ...
    char esc[512];
    int tr   = 0;
    size_t e = json_escape_into_ex(esc, sizeof esc - 1, (const uint8_t *)aggs[i].key, strlen(aggs[i].key), &tr);
    esc[e]   = 0;
    if (tr)
      break;

    if (!jout_try_printf(&w, "{\"key\":\"%s\",\"count\":%" PRIu64 "}", esc, aggs[i].count))
      break;

    size_t rl = strlen(rec);
    if (!jout_try_mem(&out, rec, rl))
      break;
  }
  ...
  (void)jout_try_printf(&out,
                        "],\"source\":{\"topic\":\"%s\",\"group\":\"%s\",\"partition\":%d,\"next_offset\":%" PRIu64
                        ",\"consumed\":%zu}}",
...
static inline int extract_key_composite(json_value_t *obj, json_value_t *fields_arr, char *out, size_t cap) {
  if (!obj || obj->type != JSON_OBJECT || !fields_arr || fields_arr->type != JSON_ARRAY)
    return 0;
  ...
  for (size_t i = 0; i < fields_arr->as.array.count; i++) {
    ...
    if (fn->type != JSON_STRING)
      continue;
    char name[64];
    jv_to_cstr2(fn, name, sizeof name);
    json_value_t *v = json_get_field(obj, name);
    if (!v)
      return 0; /* strict: missing field fails key extraction */
    char piece[256];
    if (!jv_to_cstr2(v, piece, sizeof piece))
      return 0;
    if (!first) {
      if (p < end)
        *p++ = '|';
...
static inline int build_key(json_value_t *obj, json_value_t *keys_arr, const char *single_key, char *out, size_t cap) {
  if (keys_arr && keys_arr->type == JSON_ARRAY)
    return extract_key_composite(obj, keys_arr, out, cap);
  if (single_key && *single_key)
    return extract_key_single(obj, single_key, out, cap);
  return 0;
}

static inline void write_projected_object_or_missing(char **pp, char *end, json_value_t *obj, json_value_t *fields,
                                                     int missing_as_empty) {
  char *p = *pp;
  if (!obj) {
    if (missing_as_empty) {
      if (p < end)
  ...
  if (fields && fields->type == JSON_ARRAY && fields->as.array.count > 0) {
    for (size_t i = 0; i < fields->as.array.count; i++) {
      ...
      if (fv->type == JSON_STRING) {
        char esc[512];
        size_t e = json_escape_into(esc, sizeof esc - 1, (const uint8_t *)fv->as.s.ptr, fv->as.s.len);
        esc[e]   = 0;
...
int broker_join_handler(Request *req, Response *res) {
  ...
  int lp            = 0;
  uint64_t loff     = (uint64_t)-1;
  size_t llim       = 500;
  json_value_t *ltv = json_get_field(jl, "topic");
  json_value_t *lpg = json_get_field(jl, "partition");
  json_value_t *lgg = json_get_field(jl, "group");
  json_value_t *lof = json_get_field(jl, "offset");
  ...
  if (lgg && lgg->type == JSON_STRING)
    jv_to_cstr2(lgg, lg, sizeof lg);
  if (lll && lll->type == JSON_INT && lll->as.i > 0 && lll->as.i <= 2000)
    llim = (size_t)lll->as.i;
  if (lof && (lof->type == JSON_INT || lof->type == JSON_DOUBLE)) {
    ...
    uint64_t committed = 0;
    if (rf_offset_load(lt, lg, lp, &committed))
      loff = committed;
  }
  ...
  int rp            = 0;
  uint64_t roff     = (uint64_t)-1;
  size_t rlim       = 500;
  json_value_t *rtv = json_get_field(jr, "topic");
  json_value_t *rpg = json_get_field(jr, "partition");
  json_value_t *rgg = json_get_field(jr, "group");
  json_value_t *rof = json_get_field(jr, "offset");
  ...
  if (rgg && rgg->type == JSON_STRING)
    jv_to_cstr2(rgg, rg, sizeof rg);
  if (rll && rll->type == JSON_INT && rll->as.i > 0 && rll->as.i <= 2000)
    rlim = (size_t)rll->as.i;
  if (rof && (rof->type == JSON_INT || rof->type == JSON_DOUBLE)) {
    ...
    uint64_t committed = 0;
    if (rf_offset_load(rt, rg, rp, &committed))
      roff = committed;
  }
  ...
  if ((!lkeys && (!lk1 || lk1->type != JSON_STRING)) || (!rkeys && (!rk1 || rk1->type != JSON_STRING))) {
  ...
  if (lk1 && lk1->type == JSON_STRING)
    jv_to_cstr2(lk1, left_key_single, sizeof left_key_single);
  if (rk1 && rk1->type == JSON_STRING)
    jv_to_cstr2(rk1, right_key_single, sizeof right_key_single);
  ...
  if (jwin && jwin->type == JSON_OBJECT) {
    ...
    if (sz && sz->type == JSON_INT && sz->as.i > 0)
      size_ms = (uint64_t)sz->as.i;
    if (al && al->type == JSON_INT && al->as.i >= 0)
      lateness_ms = (uint64_t)al->as.i;
  }
  ...
  if (jjoin && jjoin->type == JSON_OBJECT) {
    ...
    if (jt && jt->type == JSON_STRING) {
      char tbuf[16];
      jv_to_cstr2(jt, tbuf, sizeof tbuf);
      if (!strcmp(tbuf, "left"))
        join_type = 1;
      else if (!strcmp(tbuf, "right"))
        join_type = 2;
      else if (!strcmp(tbuf, "full"))
        join_type = 3;
      else
        join_type = 0;
    }
  ...
  // dedupe_once
  int dedupe_once = 0;
  if (jded_once && jded_once->type == JSON_BOOL)
    dedupe_once = jded_once->as.b ? 1 : 0;
  if (jded && jded->type == JSON_OBJECT) {
    json_value_t *mode = json_get_field(jded, "mode");
    if (mode && mode->type == JSON_STRING) {
      char mbuf[16];
      jv_to_cstr2(mode, mbuf, sizeof mbuf);
      if (!strcmp(mbuf, "once"))
        dedupe_once = 1;
    }
  }
  ...
  if (jemit) {
    ...
    if (mr && mr->type == JSON_INT && mr->as.i > 0 && mr->as.i <= 5000)
      max_rows = (int)mr->as.i;
    json_value_t *ma = json_get_field(jemit, "missing_as");
    if (ma && ma->type == JSON_STRING) {
      char abuf[16];
      jv_to_cstr2(ma, abuf, sizeof abuf);
      if (!strcmp(abuf, "empty"))
        missing_as_empty = 1;
    }
  }
  ...
  if (!lmsgs || !rmsgs) {
    ...
    memcpy(res->buffer, e, strlen(e));
    res->buffer[strlen(e)] = 0;
    if (lmsgs && lmsgs != lstack)
      slab_free(lmsgs);
    if (rmsgs && rmsgs != rstack)
      slab_free(rmsgs);
    json_free(root);
    return 0;
  }
  ...
  for (size_t i = 0; i < ln; i++) {
    const uint8_t *v = (const uint8_t *)lmsgs[i].val;
    size_t vl        = lmsgs[i].val_len;
    if (vl == 0 || v[0] != '{')
      continue;
    ...
    if (!build_key(obj, lkeys, left_key_single, key, sizeof key)) {
      json_free(obj);
      continue;
    }
    strncpy(L[Ln].key, key, sizeof L[Ln].key - 1);
    L[Ln].key[sizeof L[Ln].key - 1] = 0;
  ...
  for (size_t i = 0; i < rn; i++) {
    const uint8_t *v = (const uint8_t *)rmsgs[i].val;
    size_t vl        = rmsgs[i].val_len;
    if (vl == 0 || v[0] != '{')
      continue;
    json_value_t *obj = json_parse((const char *)v, vl);
    if (!obj || obj->type != JSON_OBJECT) {
    ...
    if (!build_key(obj, rkeys, right_key_single, key, sizeof key)) {
      json_free(obj);
      continue;
    }
    strncpy(R[Rn].key, key, sizeof R[Rn].key - 1);
    R[Rn].key[sizeof R[Rn].key - 1] = 0;
  ...
  size_t Lk = 0, Rk = 0;
  for (size_t i = 0; i < Ln; i++)
    if (L[i].ts_ms >= min_keep)
      L[Lk++] = L[i];
    else
      json_free(L[i].obj);
  for (size_t i = 0; i < Rn; i++)
    if (R[i].ts_ms >= min_keep)
      R[Rk++] = R[i];
    else
      json_free(R[i].obj);
  Ln = Lk;
...
#define APP(...)                                                                                                       \
  do {                                                                                                                 \
    int __n = snprintf(p, (size_t)(end - p), __VA_ARGS__);                                                             \
    if (__n < 0)                                                                                                       \
      __n = 0;                                                                                                         \
    p += (__n < (end - p) ? __n : (int)(end - p));                                                                     \
  } while (0)
  ...
  for (size_t i = 0; i < Ln && emitted < max_rows; i++) {
    ...
    for (int j = head[h]; j != -1 && emitted < max_rows; j = next[j]) {
      ...
      if (dedupe_once && !(L[i].offset <= R[j].offset))
        continue;
      ...
      if (!firstRow) {
        ...
        firstRow = 0;
      char esc[512];
      size_t e = json_escape_into(esc, sizeof esc - 1, (const uint8_t *)L[i].key, strlen(L[i].key));
      esc[e]   = 0;
      APP("{\"key\":\"%s\",\"left_ts_ms\":%" PRIu64 ",\"right_ts_ms\":%" PRIu64 ",\"left\":", esc, L[i].ts_ms,
          R[j].ts_ms);
  ...
  if ((join_type == 2 || join_type == 3) && emitted < max_rows) { /* right/full */
    for (size_t j = 0; j < Rn && emitted < max_rows; j++) {
      ...
      if (!firstRow) {
        ...
        firstRow = 0;
      char esc[512];
      size_t e = json_escape_into(esc, sizeof esc - 1, (const uint8_t *)R[j].key, strlen(R[j].key));
      esc[e]   = 0;
  ...
  APP("],\"next_offsets\":{\"left\":%" PRIu64 ",\"right\":%" PRIu64 "},"
      ...
      "\"dedupe_once\":%s}}",
      lnext, rnext, Ln, Rn, emitted, wm, outer_left_cnt, outer_right_cnt, dedupe_once ? "true" : "false");
  *p = 0;
...
int broker_produce_raw(Request *req, Response *res) {
  ...
  for (int i = 0; i < req->param_count; i++)
    if (!topic && strcmp(req->params[i].name, "topic") == 0)
      topic = req->params[i].value;

  if (!topic || !*topic) {
    send_error(res, &(error_response_t){.error   = "missing_topic",
  ...
  uint64_t part_u64 = UINT64_MAX;
  int has_part      = qs_u64(req->query_string, "partition", &part_u64);
  int partition     = has_part ? (int)part_u64 : -1;

  const char *key_q = NULL, *idk_q = NULL;
  size_t key_q_len = 0, idk_q_len = 0;
  ...
  size_t key_len = 0;
  if (key_q)
    key_len = url_decode_into(key_buf, sizeof(key_buf), key_q, key_q_len);

  char idk_buf[512];
  size_t idk_len = 0;
  if (idk_q)
    idk_len = url_decode_into(idk_buf, sizeof(idk_buf), idk_q, idk_q_len);

  const void *val_ptr = req->body;
  size_t val_len      = req->body_len;

  if (idk_len) {
    uint64_t offset = 0, batch_id = 0;
    int dup = 0;
    int rc = rf_produce_sealed_idemp(topic_buf, partition, key_len ? key_buf : NULL, key_len, val_ptr, val_len, idk_buf,
                                     idk_len, &offset, &batch_id, &dup);
    if (rc < 0) {
    ...
    uint64_t offset = 0, batch_id = 0;
    int rc = rf_produce_sealed(topic_buf, partition, key_len ? key_buf : NULL, key_len, val_ptr, val_len, &offset,
                               &batch_id);
    if (rc < 0) {
      send_error(res, &(error_response_t){
                          .error   = "produce_failed",
    ...
    int timeout_ms = 0;
    uint64_t tmp;
    if (qs_u64(req->query_string, "timeout_ms", &tmp))
      timeout_ms = (int)tmp;
    ...
    if (want_timing) {
      ...
      int n = snprintf(res->buffer, RESPONSE_BUFFER_SIZE,
                       ...
                       offset, partition, batch_id, durable ? "true" : "false", mode, synced_json, queue_us,
                       recv_parse_us, server_us, produce_us, sync_wait_us);
      ...
      int n = snprintf(res->buffer, RESPONSE_BUFFER_SIZE,
                       ...
                       "\"batch_id\":%" PRIu64 ",\"sealed\":true,"
                       "\"durable\":%s,\"mode\":\"%s\",\"synced\":%s}",
                       offset, partition, batch_id, durable ? "true" : "false", mode, synced_json);
      res->buffer[(n > 0 && n < (int)RESPONSE_BUFFER_SIZE) ? n : (RESPONSE_BUFFER_SIZE - 1)] = 0;
    }
    return 0;
  }
...
int broker_produce_ndjson(Request *req, Response *res) {
  ...
  for (int i = 0; i < req->param_count; i++)
    if (!topic && strcmp(req->params[i].name, "topic") == 0)
      topic = req->params[i].value;
  if (!topic || !*topic) {
    memcpy(res->buffer, "{\"error\":\"Missing topic\"}", 26);
    res->buffer[26] = 0;
    return -1;
  }
  ...
  const char *idk_q = NULL;
  size_t idk_q_len  = 0;
  qs_str(req->query_string, "idempotency_key", &idk_q, &idk_q_len);
  char idk_buf[512];
  size_t idk_len = idk_q ? url_decode_into(idk_buf, sizeof(idk_buf), idk_q, idk_q_len) : 0;
  ...
  if (!msgs) {
    memcpy(res->buffer, "{\"error\":\"Memory allocation failed\"}", 36);
    res->buffer[36] = 0;
    return -1;
  }
  ...
  for (const char *p = in; p <= end; p++) {
    if (p == end || *p == '\n') {
      ...
      if (len && line[len - 1] == '\r')
        len--; /* trim CR only; keep leading spaces exactly */
      ...
      if (len) {
        ...
        int looks_json      = (first == '{' || first == '[' || first == '"' || first == '-' ||
  ...
  uint64_t first_offset = 0, batch_id = 0;
  int duplicate = 0;
  int rc;
  if (idk_len) {
    rc = rf_produce_batch_sealed_idemp(topic_buf, partition, msgs, count, idk_buf, idk_len, &first_offset, &batch_id,
                                       &duplicate);
  } else {
    rc = rf_produce_batch_sealed(topic_buf, partition, msgs, count, &first_offset, &batch_id);
  }
  ...
  int n = snprintf(res->buffer, RESPONSE_BUFFER_SIZE,
                   ...
                   "\"durable\":%s,\"mode\":\"%s\",\"synced\":%s}",
                   first_offset, count, partition, batch_id, durable ? "true" : "false", mode, synced_json);
  res->buffer[(n > 0 && n < (int)RESPONSE_BUFFER_SIZE) ? n : (RESPONSE_BUFFER_SIZE - 1)] = 0;
  return 0;
}
int broker_consume(Request *req, Response *res) {
  ...
  strncpy(topic_buf, topic_in, sizeof(topic_buf) - 1);
  topic_buf[sizeof(topic_buf) - 1] = 0;
  strncpy(group_buf, group_in, sizeof(group_buf) - 1);
  group_buf[sizeof(group_buf) - 1] = 0;

  int partition    = 0;
  const char *scan = partition_str;
  while (*scan >= '0' && *scan <= '9') {
    partition = partition * 10 + (*scan - '0');
    scan++;
  }
  ...
  uint64_t next_offset = 0;
  size_t consumed = rf_consume(topic_buf, group_buf, partition, from_exclusive, max_msgs, msgs, max_msgs, &next_offset);
  ...
  rw_printf(&w, "],\"count\":%zu,\"next_offset\":%" PRIu64 ",\"committed_offset\":", consumed, next_offset);
  if (have_committed)
    rw_printf(&w, "%" PRIu64, committed);
  else
    rw_str(&w, "null");
...
int broker_commit_fast(Request *req, Response *res) {
  json_value_t *root = json_parse(req->body, req->body_len);
  if (!root || root->type != JSON_OBJECT) {
    send_error(res, &(error_response_t){.error   = "invalid_json",
  ...
  if (!topic_field || !group_field || !partition_field || !offset_field || topic_field->type != JSON_STRING ||
      group_field->type != JSON_STRING || partition_field->type != JSON_INT || offset_field->type != JSON_INT) {
    send_error(res, &(error_response_t){
  ...
  if (group_len >= sizeof(group_buf))
    group_len = sizeof(group_buf) - 1;
  ...
  if (result < 0) {
    send_error(res,
               &(error_response_t){.error   = "commit_failed",
...
int broker_create_topic_fast(Request *req, Response *res) {
  ...
  if (result < 0) {
    ...
    switch (result) {
    case -2:
      error = "{\"error\":\"Too many topics\"}";
      break;
    case -3:
      error = "{\"error\":\"Memory allocation failed\"}";
      break;
    case -4:
      error = "{\"error\":\"Partition init failed\"}";
      break;
    default:
      error = "{\"error\":\"Topic creation failed\"}";
      break;
    }
    size_t error_len = strlen(error);
    memcpy(res->buffer, error, error_len);
  ...
  int n = snprintf(res->buffer, RESPONSE_BUFFER_SIZE, "{\"ok\":true,\"topic\":\"%s\",\"partitions\":%d}", topic_buf,
                   partitions);
  res->buffer[(n > 0 && n < (int)RESPONSE_BUFFER_SIZE) ? n : (RESPONSE_BUFFER_SIZE - 1)] = '\0';
...
int sealed_status_handler(Request *req, Response *res) {
  ...
  int n = snprintf(res->buffer, RESPONSE_BUFFER_SIZE, "{\"ok\":true,\"last_synced_batch_id\":%" PRIu64 "}", last);
  res->buffer[(n > 0 && n < (int)RESPONSE_BUFFER_SIZE) ? n : (RESPONSE_BUFFER_SIZE - 1)] = '\0';
  return 0;
}
...
int sealed_gc_handler(Request *req, Response *res) {
  json_value_t *root = json_parse(req->body, req->body_len);
  if (!root || root->type != JSON_OBJECT) {
    memcpy(res->buffer, "{\"error\":\"Invalid JSON\"}", 24);
    res->buffer[24] = 0;
    if (root)
      json_free(root);
    return -1;
  ...
  if (rc < 0) {
    memcpy(res->buffer, "{\"error\":\"sealed_gc_failed\"}", 28);
    res->buffer[28] = 0;
    json_free(root);
    return -1;
  }
...
int broker_set_retention_fast(Request *req, Response *res) {
  ...
  if (ttl_f && ttl_f->type == JSON_INT) {
    ...
    int part       = -1;
    if (part_f && part_f->type == JSON_INT)
      part = (int)part_f->as.i;
    ...
    if (!have_topic || (topic_f->as.s.len == 1 && topic_f->as.s.ptr[0] == '*')) {
      ...
      if (n >= sizeof(rec.topic))
        n = sizeof(rec.topic) - 1;
  ...
  int n = snprintf(res->buffer, RESPONSE_BUFFER_SIZE,
                   ...
                   ttl_applied ? "true" : "false", size_applied ? "true" : "false", sealed_after);
  res->buffer[(n > 0 && n < (int)RESPONSE_BUFFER_SIZE) ? n : (RESPONSE_BUFFER_SIZE - 1)] = '\0';
...
int broker_delete_before_offset(Request *req, Response *res) {
  ...
  if (!topic_field || !partition_field || !before_offset_field || topic_field->type != JSON_STRING ||
      partition_field->type != JSON_INT || before_offset_field->type != JSON_INT) {
  ...
  if (rc < 0) {
    const char *error =
        (rc == -2) ? "{\"error\":\"Topic or partition not found\"}" : "{\"error\":\"Delete-before failed\"}";
    memcpy(res->buffer, error, strlen(error));
    res->buffer[strlen(error)] = '\0';
    json_free(root);
    return -1;
  ...
  int n = snprintf(res->buffer, RESPONSE_BUFFER_SIZE,
                   "{\"ok\":true,\"deleted_count\":%" PRIu64 ",\"freed_bytes\":%" PRIu64 "}", deleted, freed);
  res->buffer[(n > 0 && n < (int)RESPONSE_BUFFER_SIZE) ? n : (RESPONSE_BUFFER_SIZE - 1)] = '\0';
...
int kv_get_handler(Request *req, Response *res) {
  ...
  for (int i = 0; i < req->param_count; i++) {
    if (!ns && strcmp(req->params[i].name, "ns") == 0)
      ns = req->params[i].value;
    if (!key && strcmp(req->params[i].name, "key") == 0)
      key = req->params[i].value;
  }
  if (!ns || !key) {
    send_error(res, &(error_response_t){.error   = "missing_params",
  ...
  if (!shared_storage_get_fast(g_shared_storage, skey, buf, sizeof(buf))) {
    send_error(res, &(error_response_t){
                        .error = "not_found", .message = "Key does not exist in this namespace", .docs = NULL});
    return 0;
  }
  ...
  if (r->flags & KV_FLAG_TOMBSTONE) {
    send_error(
        res, &(error_response_t){.error = "not_found", .message = "Key was deleted (tombstone exists)", .docs = NULL});
    return 0;
  }
  ...
  if ((size_t)(w.end - w.p) < overhead) {
    send_error(res, &(error_response_t){.error   = "response_too_large",
...
int kv_meta_handler(Request *req, Response *res) {
  ...
  if (!ns || !key) {
    strcpy(res->buffer, "{\"error\":\"bad_params\"}");
    return 0;
  }
  ...
  if (r->expires_us && r->expires_us <= now) {
    strcpy(res->buffer, "{\"error\":\"not_found\",\"expired\":true}");
    return 0;
  }
  uint64_t ttl_ms = (r->expires_us && r->expires_us > now) ? (r->expires_us - now) / 1000ULL : 0;
  int n = snprintf(res->buffer, RESPONSE_BUFFER_SIZE, "{\"cas\":%" PRIu64 ",\"ttl_ms\":%" PRIu64 "}", r->cas, ttl_ms);
  (void)n;
  return 0;
}
...
int kv_put_handler(Request *req, Response *res) {
  ...
  if (!ns || !key) {
    strcpy(res->buffer, "{\"ok\":false,\"error\":\"bad_params\"}");
    return 0;
  }
  ...
  if (req->query_string) {
    uint64_t tmp;
    if (qs_u64(req->query_string, "cas", &tmp))
      want_cas = tmp;
    if (qs_u64(req->query_string, "ttl_ms", &tmp))
      ttl_ms = tmp;
  }
  ...
  if (want_cas != UINT64_MAX && want_cas != cur_cas) {
    int n = snprintf(res->buffer, RESPONSE_BUFFER_SIZE,
                     ...
                     (unsigned long long)want_cas, (unsigned long long)cur_cas, (unsigned long long)want_cas,
  ...
  if (req->body_len)
    memcpy(nw->value, req->body, req->body_len);
  ...
  {
    aof_kv_put_t hdr = {
        .skey = skey, .cas = nw->cas, .expires_us = nw->expires_us, .flags = nw->flags, .value_len = nw->value_len};
    size_t payload_sz = sizeof(hdr) + nw->value_len;
    void *payload     = alloca(payload_sz);
    memcpy(payload, &hdr, sizeof(hdr));
    if (nw->value_len)
      memcpy((uint8_t *)payload + sizeof(hdr), nw->value, nw->value_len);
...
int kv_delete_handler(Request *req, Response *res) {
  ...
  if (req->query_string)
    qs_u64(req->query_string, "cas", &want_cas);
