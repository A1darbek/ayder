...
static int write_membership_json(Response *res, const ha_membership_view_t *mv) {
    if (!res || !mv) return -1;

    char *p = res->buffer;
    char *end = res->buffer + RESPONSE_BUFFER_SIZE;

#define APP_M(...) do { \
        if (p < end) { \
            int _n = snprintf(p, (size_t)(end - p), __VA_ARGS__); \
            if (_n < 0) _n = 0; \
            if ((size_t)_n >= (size_t)(end - p)) p = end; else p += _n; \
        } \
    } while (0)

    APP_M("{\"ok\":true,\"membership\":{");
    APP_M("\"epoch\":%" PRIu64 ",\"joint_active\":%s,", mv->epoch, mv->joint_active ? "true" : "false");
    APP_M("\"voters_old_mask\":%" PRIu64 ",\"voters_new_mask\":%" PRIu64 ",\"learners_mask\":%" PRIu64 ",",
          mv->voters_old_mask, mv->voters_new_mask, mv->learners_mask);
    APP_M("\"node_count\":%u,\"nodes\":[", mv->node_count);

    int first = 1;
    for (int i = 0; i < HA_MAX_NODES; i++) {
        ha_node_config_t node = mv->nodes[i];
        if (!node.node_id[0]) continue;
        if (!first) APP_M(",");
        first = 0;
        int is_voter = (mv->voters_new_mask & (1ULL << i)) ? 1 : 0;
        int is_learner = (mv->learners_mask & (1ULL << i)) ? 1 : 0;
        APP_M("{\"index\":%d,\"node_id\":\"%s\",\"advertise_addr\":\"%s\",\"http_addr\":\"%s\",\"priority\":%d,\"is_voter\":%s,\"is_learner\":%s}",
              i, node.node_id, node.advertise_addr, node.http_addr, node.priority,
              is_voter ? "true" : "false", is_learner ? "true" : "false");
    }
    APP_M("]}}");
...
static int ha_membership_send_error(Response *res, int rc, const char *error, const char *message) {
...
static int ha_membership_map_rc(Response *res, int rc, const char *fallback_msg) {
    if (rc == 0) return 0;
    if (rc == -2) return ha_membership_send_error(res, -4, "change_in_progress", "Another membership change is already in progress");
    if (rc == -3) return ha_membership_send_error(res, -3, "no_quorum", "Leader has no fresh quorum");
    if (rc == -4) return ha_membership_send_error(res, -3, "not_leader", "Membership mutations are leader-only");
    if (rc == -5) return ha_membership_send_error(res, -5, "commit_timeout", "Membership change timed out waiting for commit");
    return ha_membership_send_error(res, -3, "membership_change_failed", fallback_msg);
}


int ha_membership_get_handler(Request *req, Response *res) {
    (void)req;
    ha_membership_view_t mv;
    if (RAMForge_HA_get_membership(&mv) != 0) {
        return ha_membership_send_error(res, -3, "ha_not_enabled", "HA membership is not available on this node");
    }
    return write_membership_json(res, &mv);
}

int ha_membership_add_handler(Request *req, Response *res) {
    json_value_t *root = json_parse(req->body, req->body_len);
    if (!root || root->type != JSON_OBJECT) {
        if (root) json_free(root);
        return ha_membership_send_error(res, -6, "invalid_json", "Body must be JSON object");
    }

    json_value_t *idv  = json_get_field(root, "node_id");
    json_value_t *adv  = json_get_field(root, "advertise_addr");
    json_value_t *http = json_get_field(root, "http_addr");
    json_value_t *pri  = json_get_field(root, "priority");

    if (!idv || idv->type != JSON_STRING || !adv || adv->type != JSON_STRING) {
        json_free(root);
        return ha_membership_send_error(res, -6, "bad_request_fields", "node_id and advertise_addr are required strings");
    }

    ha_node_config_t node;
    memset(&node, 0, sizeof(node));
    jv_to_cstr(idv, node.node_id, sizeof(node.node_id));
    jv_to_cstr(adv, node.advertise_addr, sizeof(node.advertise_addr));
    if (http && http->type == JSON_STRING) {
        jv_to_cstr(http, node.http_addr, sizeof(node.http_addr));
    } else {
        char host[256] = {0};
        int port = 0;
        if (sscanf(node.advertise_addr, "%255[^:]:%d", host, &port) == 2) {
            snprintf(node.http_addr, sizeof(node.http_addr), "http://%s:%d", host, port + 1);
        }
    }
    node.priority = (pri && (pri->type == JSON_INT || pri->type == JSON_DOUBLE))
                    ? (int)(pri->type == JSON_INT ? pri->as.i : pri->as.d)
                    : 10;
    node.is_voter = 0;

    int idx = -1;
    int rc = RAMForge_HA_add_member(&node, &idx);
    json_free(root);

    if (rc != 0) {
        return ha_membership_map_rc(res, rc, "Failed to add learner member");
    }

    snprintf(res->buffer, RESPONSE_BUFFER_SIZE,
             "{\"ok\":true,\"index\":%d,\"node_id\":\"%s\",\"role\":\"learner\"}",
             idx, node.node_id);
    res->buffer[RESPONSE_BUFFER_SIZE - 1] = '\0';
    return 0;
}

int ha_membership_promote_handler(Request *req, Response *res) {
    json_value_t *root = json_parse(req->body, req->body_len);
    if (!root || root->type != JSON_OBJECT) {
        if (root) json_free(root);
        return ha_membership_send_error(res, -6, "invalid_json", "Body must be JSON object");
    ...
    json_value_t *idv = json_get_field(root, "node_id");
    if (!idv || idv->type != JSON_STRING) {
        json_free(root);
        return ha_membership_send_error(res, -6, "bad_request_fields", "node_id is required string");
    }

    char node_id[64];
    jv_to_cstr(idv, node_id, sizeof(node_id));
    json_free(root);

    int rc = RAMForge_HA_promote_member(node_id);
    if (rc != 0) {
        return ha_membership_map_rc(res, rc, "Failed to promote learner");
    }

    snprintf(res->buffer, RESPONSE_BUFFER_SIZE,
             "{\"ok\":true,\"node_id\":\"%s\",\"role\":\"voter\"}", node_id);
    res->buffer[RESPONSE_BUFFER_SIZE - 1] = '\0';
    return 0;
}

int ha_membership_remove_handler(Request *req, Response *res) {
    json_value_t *root = json_parse(req->body, req->body_len);
    if (!root || root->type != JSON_OBJECT) {
        if (root) json_free(root);
        return ha_membership_send_error(res, -6, "invalid_json", "Body must be JSON object");
    ...
    int rc = RAMForge_HA_remove_member(node_id);
    if (rc != 0) {
        return ha_membership_map_rc(res, rc, "Failed to remove member");
    }

    snprintf(res->buffer, RESPONSE_BUFFER_SIZE,
             "{\"ok\":true,\"node_id\":\"%s\",\"removed\":true}", node_id);
    res->buffer[RESPONSE_BUFFER_SIZE - 1] = '\0';
    return 0;
}
...
void register_application_routes(App *app, int control_plane) {
    ...
    app->get(app, "/ha/membership", ha_membership_get_handler);
    app->post(app, "/ha/membership/add", ha_membership_add_handler);
    app->post(app, "/ha/membership/promote", ha_membership_promote_handler);
    app->post(app, "/ha/membership/remove", ha_membership_remove_handler);
