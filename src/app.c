...
static void router_get_wrap(struct App *self, const char *path, RouteHandler h) {
  (void)self; /* storage not needed here */
  register_route("GET", path, h);
}

static void router_post_wrap(struct App *self, const char *path, RouteHandler h) {
  (void)self;
  register_route("POST", path, h);
}

static void router_delete_wrap(struct App *self, const char *path, RouteHandler h) {
  (void)self;
  register_route("DELETE", path, h);
}

/* stub (HTTP server lives in cluster.c) */
static void app_start_http(struct App *self, int port) { http_server_init(self, port); }
