...
extern void RAMForge_configure_rotation_policy(size_t max_rdb_mb, time_t max_age_hours, int keep_count,
                                               int chaos_enabled);
...
static void cluster_signal_handler(int sig) {
  ...
  if (shut_once)
    return;
  shut_once = 1;
  printf("\033[2K\r"); // Clear current line in terminal
  fflush(stdout);
  LOGW("Cluster manager caught signal %d – shutting down workers ...", sig);
  cluster_shutdown = 1;
  if (!worker_pids || worker_count <= 0)
    return;
  for (int i = 0; i < worker_count; i++)
    if (worker_pids[i] > 0) {
      LOGI("SIGTERM → worker %d (PID %d)", i, worker_pids[i]);
...
static void single_process_signal_handler(int sig) {
  LOGW("Single process mode caught signal %d – shutting down gracefully ...", sig);
  cluster_shutdown = 1;
}
...
static void setup_cpu_affinity(int wid) {
  ...
  if (ncpu == 1) {
    ...
    int slot = wid;
    if (ded_wid >= 0 && wid > ded_wid)
      slot--; // close the “gap” after removing dedicated worker
    cpu = 1 + (slot % (ncpu - 1));
  }
...
static App *init_worker_systems(int wid) {
  ...
  if (!g_shared_storage) {
    ...
    if (!g_shared_storage) {
      LOGE("Worker %d failed to attach to shared storage", wid);
      return NULL;
    }
    if (single_process_mode || wid == 0) {
      LOGI("Shared storage attached (1M entries available)");
    }
  }
  const char *aof = "./append.aof";

  if (single_process_mode) {
    LOGI("Single process mode using AOF: %s", aof);
  }
  ...
  if (aof_enabled) {
    ...
    if (single_process_mode || wid == 0) {
      LOGI("AOF persistence enabled (flush_ms=%u)", enhanced_flush_ms);
      LOGI("Ring capacity: 128K entries");
      LOGI("Buffer pool: 2048 aligned buffers");
      LOGI("Batch size: 256 operations");
    }
  } else {
    if (single_process_mode || wid == 0) {
      LOGI("AOF persistence disabled");
    }
  }
  ...
  if (!app) {
    LOGE("app_create failed (worker %d)", wid);
    return NULL;
  }
...
static void run_worker(int wid, int port) {
  ...
  const char *ded  = getenv("RF_HA_DEDICATED_WORKER");
  if (ded && *ded)
    ha_dedicated = (atoi(ded) == wid);

  const char *ha_en = getenv("RF_HA_ENABLED");
  if (ha_en && ha_en[0] == '1') {
    if (wid > 0)
      setenv("RF_HA_AGENT_ONLY", "1", 1);
    LOGI("Worker %d initializing HA (%s)...", wid, (wid == 0 ? "leader-runtime" : "agent-only"));
    if (RAMForge_HA_init() != 0) {
      LOGW("Worker %d HA init failed - continuing without HA", wid);
    }
  }
  ...
  if (!ha_dedicated) {
    ...
    LOGW("Worker %d is HA-dedicated: not listening on data port %d", wid, port);
    for (;;)
      pause(); // keep process alive; HA threads keep running
  }
...
static void wait_for_workers(void) {
  ...
  while (left > 0) {
    ...
    if (pid > 0) {
      ...
      if (WIFEXITED(st))
        LOGI("Worker %d (PID %d) exited code %d", wid, pid, WEXITSTATUS(st));
      else if (WIFSIGNALED(st))
        LOGW("Worker %d (PID %d) killed by signal %d", wid, pid, WTERMSIG(st));
      left--;
    } else if (errno == EINTR)
      continue;
    else
...
int start_cluster_with_args(int port, int argc, char **argv) {
  ...
  const char *e;
  if ((e = getenv("RF_BUS_HEAP_MB")) && *e)
    bus_heap = (size_t)strtoull(e, NULL, 10) * 1024ULL * 1024ULL;
  if ((e = getenv("RF_BUS_DESC")) && *e)
    bus_desc = (size_t)strtoull(e, NULL, 10);

  rf_bus_init_pre_fork(bus_heap, bus_desc);

  /* FIXED - if caller requests 0 workers, run a single worker in-process */
  if (worker_count == 0) {
    LOGI("Single-process mode (no cluster manager)");
  ...
  if (!worker_pids) {
    LOGE("Failed to allocate memory for worker PIDs");
    return -1;
  }

  LOGI("Starting Ayder with %d worker%s on port %d", worker_count, worker_count == 1 ? "" : "s", port);
  ...
  LOGI("All workers live – monitoring ... (Ctrl-C to stop)");
  /* monitor loop */
  while (!cluster_shutdown) {
    ...
    if (dead > 0) {
      ...
      int fatal = (WIFEXITED(st) && WEXITSTATUS(st) != 0) || WIFSIGNALED(st);

      if (fatal) {
        LOGW("Worker %d (PID %d) exited abnormally – shutting cluster down", wid, dead);
        cluster_shutdown = 1;
        ...
        LOGI("Worker %d exited normally – stopping cluster", wid);
        cluster_shutdown = 1;
  ...
  LOGI("Cluster shutting down – waiting for workers ...");
  wait_for_workers();
  free(worker_pids);
  LOGI("Cluster shutdown complete");
  return 0;
}
