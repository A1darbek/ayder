...
static inline uint32_t clamp_cap_pow2(uint32_t cap) {
    ...
    if (cap > MAX_SHARED_ENTRIES_MAX) cap = MAX_SHARED_ENTRIES_MAX;

    /* ensure pow2 */
    if (cap & (cap - 1)) cap = floor_pow2_u32(cap);
    if (cap < MIN_CAP) cap = MIN_CAP;
    return cap;
}
        ...
        if (budget > overhead + (unsigned long long)sizeof(SharedEntry)) {
            ...
            while (cap > 0) {
                unsigned long long need =
                        overhead + (unsigned long long)cap * (unsigned long long)sizeof(SharedEntry);
                if (need <= budget) break;
                cap >>= 1;
            }

            if (cap) return clamp_cap_pow2(cap);
        }
    }
...
static int shm_has_space(size_t need_bytes) {
    struct statvfs v;
    if (statvfs("/dev/shm", &v) != 0) return 1; /* if unknown, don't block */
...
SharedStorage *shared_storage_init_named(const char *name) {
    if (!name || !*name) name = SHARED_STORAGE_NAME;
    ...
    if (!shm_has_space(shm_sz)) {
        fprintf(stderr,
                "❌ /dev/shm too small for SharedStorage (%zu bytes ≈ %.1f MiB). "
    ...
    if (init_mutex_attrs(&mattr) != 0) {
        fprintf(stderr, "pthread_mutexattr init failed\n");
        munmap(ss, shm_sz);
        shm_unlink(name);
        return NULL;
    }
...
SharedStorage *shared_storage_attach(void) {
    return shared_storage_attach_named(SHARED_STORAGE_NAME);
}
...
int shared_storage_set(SharedStorage *ss, int key, const void *data, size_t sz) {
    if (!ss || !data || sz > MAX_DATA_SIZE || key == 0) return -1;
...
void shared_storage_iterate(SharedStorage *ss,
    ...
    for (uint32_t i = 0; i < cap; ++i) {
        ...
        if (k && sz) cb(k, ss->entries[i].data, sz, ud);
    }
}
...
void shared_storage_stats(SharedStorage *ss) {
    ...
    uint32_t cap = capacity_load(ss);
    fprintf(stderr,
            "SharedStorage: cap=%u size=%u reads=%llu writes=%llu collisions=%llu shm=%.1f MiB\n",
            cap,
            (unsigned)atomic_load(&ss->size),
            (unsigned long long)atomic_load(&ss->read_ops),
            (unsigned long long)atomic_load(&ss->write_ops),
            (unsigned long long)atomic_load(&ss->collisions),
