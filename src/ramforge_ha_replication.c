...
static void leader_maybe_advance_commit(void) {
  ...
  arr[m++] = last_local;
  for (int i = 0; i < n; i++) {
    if (i == g_config->local_node_index)
      continue;
  ...
  if (cand > atomic_load(&g_runtime->commit_index) && term_at(cand) == atomic_load(&g_runtime->term)) {
    atomic_store(&g_runtime->commit_index, cand);
  }
}

int HA_leader_on_ack(int peer_index, uint64_t match_index) {
  if (!HA_is_leader(g_runtime))
    return -1;

  /* monotonic */
  uint64_t cur = atomic_load(&g_runtime->match_index[peer_index]);
  while (match_index > cur && !atomic_compare_exchange_weak(&g_runtime->match_index[peer_index], &cur, match_index)) {
  }
...
static ha_log_entry_t *create_log_entry(uint64_t index, uint32_t type, const void *data, uint32_t size) {
...
static int connect_to_peer(int peer_index) {
  if (!g_config)
    return -1;
  if (peer_index == g_config->local_node_index)
    return -1;
  if (peer_index < 0 || peer_index >= g_config->node_count)
    return -1;
  ...
  LOGI("[HA-CONNECT] Connected to peer %d (%s:%d) fd=%d", peer_index, host, port, fd);
...
static size_t pack_entries(uint64_t from, uint32_t max, uint8_t *dst, size_t cap, uint64_t *last_idx_out) {
  size_t used = 0;
  uint64_t i  = from;
...
static void send_heartbeat_with_entries(void) {
  ...
  for (int peer = 0; peer < n; peer++) {
    ...
    for (int batch = 0; batch < batches_this_peer; batch++) {
      if (budget_us && (now_us() - start) > budget_us)
        break;
      ...
      size_t payload_sz  = pack_entries(next, batch_size, payload, buf_max - sizeof(*ae), &last_sent);

      if (payload_sz) {
        ha_msg_header_t h = {.type = HA_MSG_APPEND_ENTRIES, .term = atomic_load(&g_runtime->term)};
...
static void start_election(void) {
  ...
  LOGI("Starting election for term %lu", new_term);
  ...
  for (int i = 0; i < g_config->node_count; i++) {
    if (i == g_config->local_node_index)
      continue;
    if (!g_config->nodes[i].is_voter)
      continue;

    ha_msg_header_t header = {.type = HA_MSG_VOTE_REQUEST, .term = new_term};

    ha_vote_request_t request = {.last_log_index = last_index,
  ...
  while ((now_us() - start) < timeout_us) {
    if (atomic_load(&g_runtime->votes_granted) >= votes_needed) {
      ...
      if (atomic_load(&g_runtime->term) == new_term) {
        ...
        LOGI("Won election, now LEADER for term %lu", new_term);
      }
  ...
  LOGW("Election timeout for term %lu", new_term);
}
...
int HA_replication_init(ha_config_t *config, ha_runtime_t *runtime) {
  ...
  LOGI("HA replication engine initialized");
  return 0;
}
...
void HA_replication_shutdown(void) {
  ...
  LOGI("HA replication engine shutdown complete");
}
...
uint64_t HA_repl_kick_read(void) { return atomic_load(&g_repl_kick); }

int HA_forward_append_and_replicate(uint32_t rec_type, const void *data, size_t size) {
  if (!g_config || !g_runtime)
    return -1;
  if (!HA_is_leader(g_runtime))
    return -1;
...
void HA_get_log_stats(uint64_t *last_index, uint64_t *committed, uint64_t *applied) {
  if (last_index)
    *last_index = atomic_load(&g_log_ring.head) - 1;
  if (committed)
    *committed = atomic_load(&g_runtime->commit_index);
  if (applied)
    *applied = atomic_load(&g_runtime->applied_index);
}
