...
static void tls_log_errors(const char *where) {
  unsigned long e;
  LOGE("TLS error at %s:", where ? where : "(unknown)");
...
int HA_tls_enabled(void) { return atomic_load(&g_tls_enabled); }
...
int HA_tls_init_from_env(void) {
  ...
  if (!ca || !*ca || !cert || !*cert || !key || !*key) {
    LOGE("RF_HA_TLS=1 but RF_HA_TLS_CA/CERT/KEY not fully set");
    return -1;
  }

  const char *strict = getenv("RF_HA_TLS_STRICT_ID");
  if (strict && strict[0] == '0')
    atomic_store(&g_tls_strict_id, 0);

  const char *tmo = getenv("RF_HA_TLS_TIMEOUT_MS");
  if (tmo && *tmo) {
    int v = atoi(tmo);
    if (v > 0)
      atomic_store(&g_tls_timeout_ms, v);
  }
  ...
  if (SSL_CTX_set_cipher_list(g_server_ctx, "ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-RSA-AES256-GCM-SHA384:"
                                            "ECDHE-ECDSA-CHACHA20-POLY1305:ECDHE-RSA-CHACHA20-POLY1305:"
                                            "ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-GCM-SHA256") != 1) {
    tls_log_errors("SSL_CTX_set_cipher_list(server)");
    return -1;
  }
  if (SSL_CTX_set_cipher_list(g_client_ctx, "ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-RSA-AES256-GCM-SHA384:"
                                            "ECDHE-ECDSA-CHACHA20-POLY1305:ECDHE-RSA-CHACHA20-POLY1305:"
                                            "ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-GCM-SHA256") != 1) {
  ...
  LOGI("HA mTLS enabled (strict_id=%d timeout_ms=%d)", atomic_load(&g_tls_strict_id), atomic_load(&g_tls_timeout_ms));
  return 0;
}
...
SSL *HA_tls_new_client(int fd, const char *sni_host) {
  if (!HA_tls_enabled() || !g_client_ctx)
    return NULL;
...
SSL *HA_tls_new_server(int fd) {
  if (!HA_tls_enabled() || !g_server_ctx)
    return NULL;
...
int HA_tls_handshake(SSL *ssl, int fd, int is_server, int timeout_ms) {
  ...
  for (;;) {
    if (++attempts > max_attempts) {
      LOGW("TLS handshake timeout after %d attempts", attempts);
      return -1;
    }
...
static int cert_matches_expectations(X509 *cert, const char *node_id, const char *adv_host) {
  ...
  if (node_id && *node_id) {
    if (X509_check_host(cert, node_id, 0, 0, NULL) == 1)
      ok = 1;
  }
  ...
  if (!ok && adv_host && *adv_host) {
    ...
    if (is_ip) {
      if (X509_check_ip_asc(cert, adv_host, 0) == 1)
        ok = 1;
    } else {
      if (X509_check_host(cert, adv_host, 0, 0, NULL) == 1)
        ok = 1;
    }
  }
...
int HA_tls_verify_peer_identity(SSL *ssl, const ha_config_t *cfg, int peer_index) {
  if (!HA_tls_enabled())
    return 1;
  if (!ssl || !cfg || peer_index < 0 || peer_index >= cfg->node_count)
    return 0;
  if (atomic_load(&g_tls_strict_id) == 0)
    return 1;
  ...
  if (!ok) {
    LOGE("TLS peer identity mismatch for peer_index=%d expect(node_id='%s' host='%s')", peer_index,
         node_id ? node_id : "(null)", host);
  }
  return ok;
}
