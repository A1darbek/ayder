...
static void HA_publish_snapshot_tick(uint64_t now_us) {
    ...
    if (snap.leader_index >= 0 &&
        snap.leader_index < g_ha_config.node_count) {
        ...
        if (HA_is_leader(&g_ha_runtime)) {
            ...
            if (now - last_election_check >= election_timeout_us) {
                if (now - last_hb >= election_timeout_us) {
                    printf("⚠️  Election timeout - no heartbeat from leader\n");
...
int RAMForge_HA_init(void) {
    ...
    if (!enabled || strcmp(enabled, "1") != 0) {
        printf("ℹ️  HA disabled (set RF_HA_ENABLED=1 to enable)\n");
    ...
    if (HA_tls_init_from_env() != 0) {
        fprintf(stderr, "❌ Failed to initialize HA mTLS (RF_HA_TLS=1)\n");
        return -1;
    }
    ...
    if (!atomic_load(&g_ha_agent_only)) {
        ...
        if (HA_net_start_server(&g_ha_config, &g_ha_runtime) != 0) {
            fprintf(stderr, "❌ Failed to start HA inbound server\n");
            return -1;
        }
    } else {
        printf("👀 HA agent-only mode: will read HA state from shared memory\n");
    }
    ...
    if (g_ha_config.node_count == 1) {
        ...
        atomic_store(&g_ha_runtime.state, HA_STATE_READY);
        printf("✅ Single-node cluster: self-electing LEADER\n");
    ...
    if (!atomic_load(&g_ha_agent_only) && g_ha_config.node_count > 1) {
        ...
        if (boot && strcmp(boot, "1") == 0) {
            ...
            atomic_store(&g_ha_runtime.current_leader, g_ha_config.local_node_index);
            atomic_store(&g_ha_runtime.state, HA_STATE_READY);
            printf("✅ Bootstrap: assuming LEADER (initial bring-up)\n");
        }
    }
...
void RAMForge_HA_shutdown(void) {
    ...
    printf("🛑 Shutting down HA system...\n");
...
int RAMForge_HA_is_leader(void) {
    if (!atomic_load(&g_ha_enabled)) return 1; // legacy default
    if (!atomic_load(&g_ha_agent_only)) return HA_is_leader(&g_ha_runtime);
    ha_shmem_state_t s;
    if (!ha_load_from_shmem(&s)) return 0; /* safe default: not leader */
...
int RAMForge_HA_is_follower(void) {
    ...
    ha_shmem_state_t s;
    if (!ha_load_from_shmem(&s)) return 1; /* safe default: follow */
...
int RAMForge_HA_replicate_write(uint32_t record_type, const void *data, uint32_t size) {
    if (!atomic_load(&g_ha_enabled)) return 0; // HA disabled - succeed immediately
...
// Get node statistics
void RAMForge_HA_get_stats(uint64_t *log_index, uint64_t *commit_index,
                           uint64_t *applied_index, uint64_t *term,
                           int *role, int *state) {
    if (!atomic_load(&g_ha_enabled)) {
        if (log_index) *log_index = 0;
        if (commit_index) *commit_index = 0;
        if (applied_index) *applied_index = 0;
        if (term) *term = 0;
        if (role) *role = HA_ROLE_LEADER;
        if (state) *state = HA_STATE_READY;
        return;
    }

    HA_get_log_stats(log_index, commit_index, applied_index);

    if (term) *term = atomic_load(&g_ha_runtime.term);
    if (role) *role = atomic_load(&g_ha_runtime.role);
    if (state) *state = atomic_load(&g_ha_runtime.state);
}
