...
static inline uint64_t rf_fetch_add_u64_relaxed(_Atomic uint64_t *obj, uint64_t inc) {
  ...
  for (;;) {
    ...
    if (atomic_compare_exchange_weak_explicit(obj, &cur, next,
...
static global_fd_state_t g_fd_state = {.master_fd         = ATOMIC_VAR_INIT(-1),
...
#define HN 10
static const double fsync_bucket_le[HN] = {0.0005, 0.001, 0.002, 0.005, 0.010,
                                           0.020,  0.050, 0.100, 0.250, 0.500}; // seconds
...
static inline void hist_obs(_Atomic uint64_t *arr, const double *le, int n, double v, _Atomic uint64_t *cnt,
                            _Atomic double *sum) {
  for (int i = 0; i < n; i++) {
    if (v <= le[i]) {
      atomic_fetch_add(&arr[i], 1);
      goto out;
    }
  }
...
out:
  ...
  do {
    neu = old + v;
  } while (!atomic_compare_exchange_weak(sum, &old, neu));
}
...
void AOF_fsync_hist_snapshot(uint64_t *b, size_t n, double *sum_s, uint64_t *count) {
  ...
  for (size_t i = 0; i < m; i++)
    b[i] = atomic_load(&g_fsync_hist[i]);
...
void AOF_seal_latency_hist_snapshot(uint64_t *b, size_t n, double *sum_s, uint64_t *count) {
  size_t m = (n < HN) ? n : HN;
  for (size_t i = 0; i < m; i++)
    b[i] = atomic_load(&g_seal_hist[i]);
  *sum_s = atomic_load(&g_seal_sum_s);
  *count = atomic_load(&g_seal_count);
}
const double *AOF_fsync_bucket_bounds(int *n) {
  *n = HN;
  return fsync_bucket_le;
}
const double *AOF_seal_bucket_bounds(int *n) {
  *n = HN;
  return seal_bucket_le;
}

static inline size_t write_sealed_batch_to_buffer(char *buf, uint64_t batch_id, uint64_t timestamp, uint32_t msg_count,
  ...
  struct {
    ...
    uint32_t total_size;
    uint32_t header_crc;
  } __attribute__((packed)) header = {.magic      = 0x5EA1ED01,
                                      .batch_id   = batch_id,
                                      .timestamp  = timestamp,
                                      .msg_count  = msg_count,
                                      .total_size = size,
...
void AOF_sealed_test_set_hold(int on) {
  atomic_store(&g_test_hold, on ? 1 : 0);
  if (on)
    atomic_store(&g_test_release, 0);
}
...
static void *sealed_sync_thread(void *arg) {
  ...
  while (!atomic_load(&g_opt_sealed.shutdown)) {
    ...
    if (written <= 0) {
      perror("ealed writev failed");
      continue;
    }

    if (fsync(g_opt_sealed.fd) != 0) {
      perror("ealed fsync failed");
      continue;
    }
...
uint64_t AOF_sealed_last_synced_id(void) { return atomic_load(&g_opt_sealed.last_synced_batch_id); }
...
int AOF_sealed_init(const char *path) {
  ...
  g_opt_sealed.fd = open(sealed_file, O_CREAT | O_APPEND | O_WRONLY | O_DIRECT | O_CLOEXEC, 0600);
  if (g_opt_sealed.fd < 0) {
    /* Fallback without O_DIRECT if not supported */
    g_opt_sealed.fd = open(sealed_file, O_CREAT | O_APPEND | O_WRONLY | O_CLOEXEC, 0600);
  }
  ...
  g_opt_sealed.mmap_buffer = mmap(NULL, SEALED_MMAP_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  ...
  if (h)
    AOF_sealed_test_set_hold(atoi(h));
  if (d)
    AOF_sealed_test_set_delay_us(atoi(d));
  ...
  if (pthread_create(&g_opt_sealed.sync_thread, NULL, sealed_sync_thread, NULL) != 0) {
    munmap(g_opt_sealed.mmap_buffer, SEALED_MMAP_SIZE);
    close(g_opt_sealed.fd);
    return -1;
  }
...
uint64_t AOF_append_sealed(int id, const void *data, size_t sz) {
  ...
  // claim slot
  int expect = 0;
  while (!atomic_compare_exchange_weak_explicit(&slot->state, &expect, 1, memory_order_acq_rel, memory_order_relaxed)) {
    expect = 0;
    cpu_relax();
  }
...
static inline uint32_t rd32_gc(const void *p) {
  uint32_t v;
  memcpy(&v, p, 4);
  return v;
}
static inline uint64_t rd64_gc(const void *p) {
  uint64_t v;
  memcpy(&v, p, 8);
  return v;
}
...
int AOF_sealed_gc(uint64_t max_age_ms, uint64_t max_keep_bytes, uint64_t *bytes_after) {
  ...
  for (;;) {
    ...
    for (int i = 0; i < MAX_SEALED_BATCHES; i++)
      if (atomic_load(&g_opt_sealed.slots[i].state) == 2) {
        any_ready = true;
        break;
      }
    if (!any_ready)
...
size_t AOF_sealed_replay(const char *aof_path) {
  ...
  if (local_fd < 0) {
    if (errno != ENOENT)
      LOGE("AOF_sealed_replay: open(%s): %s", path, strerror(errno));
    return 0;
  }
  ...
  while (p + 32 <= end) {
    ...
    if (crc_calc != crc_file) {
      LOGE("AOF_sealed_replay: CRC mismatch (id=%u, sz=%u)", rec_id, sz);
      bad++;
    } else {
  ...
  if (applied || bad || skipped) {
    LOGI("PID %d sealed replay: applied=%zu, skipped=%zu, bad=%zu (file=%s, %zu bytes)", getpid(), applied, skipped,
         bad, path, fsz);
  }
...
static void init_tls_fd_cache(void) {
  if (pthread_mutex_init(&tls_fd_cache.lock, NULL) != 0) {
    LOGE("Failed to initialize TLS fd cache mutex");
    exit(1);
  }
  tls_fd_cache.fd         = -1;
...
static int init_io_uring(void) {
  if (running_under_valgrind()) {
    LOGW("Valgrind detected — disabling io_uring for this run");
  ...
  if (ret < 0) {
    LOGE("io_uring_queue_init_params failed: %s", strerror(-ret));
    atomic_store(&g_uring_ctx.initialized, false);
    return -1;
  }
...
static void destroy_io_uring(void) {
  if (atomic_load(&g_uring_ctx.initialized)) {
    ...
    while (atomic_load(&g_uring_ctx.pending_writes) > 0) {
      ...
      if (io_uring_wait_cqe_timeout(&g_uring_ctx.ring, &cqe, &(struct __kernel_timespec){0, 1000000}) == 0) { // 1ms
    ...
    io_uring_queue_exit(&g_uring_ctx.ring);
    atomic_store(&g_uring_ctx.initialized, false);
    LOGI("io_uring destroyed");
  }
}

/* ─── Write-ahead buffer initialization ─────────── */
static int init_wal_buffer(void) {
  g_wal_buffer.buffer = mmap(NULL, WRITE_AHEAD_BUFFER_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
static int submit_uring_write(const void *buffer, size_t size) {
  ...
  if (write_fd < 0) {
    LOGE("Failed to get valid fd for write");
    return -1;
  }
  ...
  if (fcntl(write_fd, F_GETFL) == -1) {
    ...
    free(batch->buffer);
    free(batch);
    LOGE("fd became invalid during submission preparation");
    return -1;
  }
...
static void *uring_worker_thread(void *arg) {
  ...
  while (atomic_load(&running)) {
    struct io_uring_cqe *cqe;
    int ret = io_uring_wait_cqe_timeout(&g_uring_ctx.ring, &cqe, &(struct __kernel_timespec){0, 100000000}); /* 100ms */
    ...
    if (ret < 0) {
      if (ret != -EINTR) {
        LOGE("io_uring_wait_cqe error: %s", strerror(-ret));
      }
      continue;
    }
      ...
      if (cqe->res < 0) {
        if (cqe->res == -EBADF) {
          ...
          if (batch->generation < current_gen) {
            LOGW("Expected EBADF during rotation (gen %" PRIu64 " -> %" PRIu64 ")", batch->generation, current_gen);
          } else {
            LOGE("Unexpected EBADF (same generation %" PRIu64 ")", current_gen);
          }
        } else {
          LOGE("io_uring write failed: %s", strerror(-cqe->res));
        }
  ...
  LOGI("Enhanced io_uring worker thread exiting");
  return NULL;
}
...
static void *writer_thread(void *arg) {
  ...
  if (!batch_buffer) {
    LOGE("Failed to allocate batch buffer");
    return NULL;
  }
  ...
  while (atomic_load(&running)) {
    ...
    while (head != tail && batch_count < BATCH_SIZE && is_fd_valid(fd)) {
      ...
      size_t record_size = write_record_to_buffer(batch_buffer + total_batch_size, c->id, c->data, c->sz);
      ...
      if (atomic_load(&rewrite_active)) {
        ...
        if (nxt != rewrite_buf_tail) {
          ...
          if (copy) {
            ...
            rewrite_buffer[rewrite_buf_head] = (aof_cmd_t){c->id, c->sz, copy, 0, c->sequence, ATOMIC_VAR_INIT(false)};
    ...
    if (batch_count > 0) {
      if (atomic_load(&g_uring_ctx.initialized)) {
        if (submit_uring_write(batch_buffer, total_batch_size) != 0) {
          ...
          if (is_fd_valid(fd)) {
            ...
            if (w != (ssize_t)total_batch_size) {
              if (errno == EBADF) {
                LOGW("Write failed due to bad fd - attempting recovery");
                AOF_reopen_if_needed();
              } else {
        ...
        if (is_fd_valid(fd)) {
          ...
          if (w != (ssize_t)total_batch_size) {
            if (errno == EBADF) {
              LOGE("Sync write failed due to bad fd");
              AOF_reopen_if_needed();
            } else {
              perror("AOF write");
            }
  ...
  LOGI("Enhanced writer thread exiting");
  return NULL;
}
...
static void dump_record_cb(int id, const void *data, size_t sz, void *ud) {
  ...
  if (written != (ssize_t)record_size) {
    LOGE("Write error in dump_record_cb");
  }
}
...
static void *segment_rewrite_thread_func(void *arg) {
  ...
  LOGI("Starting segment rewrite: %s", task->source_path);
  ...
  if (fd_out < 0) {
    perror("egment_rewrite: open output");
    goto cleanup;
  }
  ...
  if (fd_in < 0) {
    perror("egment_rewrite: open input");
    close(fd_out);
    unlink(tmp_path);
  ...
  while (read(fd_in, &id, 4) == 4) {
    if (read(fd_in, &size, 4) != 4) {
      LOGE("segment_rewrite: corrupt size field");
      break;
    }

    void *buf = malloc(size);
    if (!buf) {
      LOGE("segment_rewrite: malloc failed");
      break;
    }

    if (read(fd_in, buf, size) != (ssize_t)size) {
      LOGE("segment_rewrite: corrupt data field");
      free(buf);
      break;
    }

    if (read(fd_in, &crc_file, 4) != 4) {
      LOGE("segment_rewrite: corrupt crc field");
      free(buf);
      break;
    }
    ...
    if (crc != crc_file) {
      LOGE("segment_rewrite: CRC mismatch");
      free(buf);
      break;
    }
  ...
  if (rename(tmp_path, task->output_path) != 0) {
    perror("egment_rewrite: rename failed");
    unlink(tmp_path);
  } else {
    LOGI("Segment rewrite complete: %s", task->output_path);
  }
...
int AOF_segment_rewrite_in_progress(void) { return atomic_load(&segment_rewrite_active); }

/* ─── Non-blocking rewrite thread (enhanced) ─────────── */
static void *rewrite_thread_func(void *arg) {
  Storage *st = (Storage *)arg;

  LOGI("Starting non-blocking AOF rewrite...");
  ...
  pthread_mutex_lock(&rewrite_lock);
  LOGI("Applying %zu buffered operations...", (rewrite_buf_head - rewrite_buf_tail) & rewrite_buf_mask);
  ...
  atomic_store(&rewrite_active, false);
  LOGI("Non-blocking AOF rewrite complete!");
  return NULL;
}
...
void AOF_init(const char *path, size_t ring_capacity, unsigned interval_ms) {
  ...
  if (init_io_uring() != 0) {
    LOGW("io_uring unavailable — falling back to sync AOF writes");
    atomic_store(&g_uring_ctx.initialized, false);
  }

  if (init_wal_buffer() != 0) {
    LOGE("Failed to initialize WAL buffer");
    exit(1);
  }
  ...
  if (!mode_always) {
    ...
    if (pthread_mutex_init(&lock, NULL) != 0 || pthread_cond_init(&cond, NULL) != 0 ||
        pthread_mutex_init(&rewrite_lock, NULL) != 0) {
      LOGE("Failed to initialize mutexes");
      exit(1);
    }
  }
  ...
  if (atomic_load(&g_uring_ctx.initialized)) {
    if (pthread_create(&uring_worker, NULL, uring_worker_thread, NULL) != 0) {
      LOGE("Failed to create safe io_uring worker thread");
      exit(1);
    }
  }
...
void AOF_append(int id, const void *data, size_t sz) {
  ...
  if (!buf) {
    LOGE("AOF_append: buffer allocation failed");
    return;
  }
  ...
  if (mode_always) {
    ...
    if (atomic_load(&g_uring_ctx.initialized)) {
      ...
      if (w != (ssize_t)record_size)
        perror("AOF direct write");
    }
...
void AOF_load(Storage *st) {
  ...
  if (fd_load < 0) {
    if (errno == ENOENT) {
      LOGI("AOF file not found, starting fresh");
      return;
    }
  ...
  while (read(fd_load, &id, 4) == 4) {
    if (read(fd_load, &size, 4) != 4) {
      LOGE("AOF_load: corrupt size field");
      break;
    }

    if (size > 16 * 1024 * 1024) { /* 16MB sanity check */
      LOGE("AOF_load: size too large: %u", size);
      break;
    }

    void *buf = malloc(size);
    if (!buf) {
      LOGE("AOF_load: malloc failed");
      break;
    }

    if (read(fd_load, buf, size) != (ssize_t)size) {
      LOGE("AOF_load: corrupt data field");
      free(buf);
      break;
    }

    if (read(fd_load, &crc_file, 4) != 4) {
      LOGE("AOF_load: corrupt crc field");
      free(buf);
      break;
    }
    ...
    if (crc != crc_file) {
      LOGE("AOF_load: CRC mismatch at record %zu", loaded);
...
void AOF_rewrite(Storage *st) {
  if (atomic_load(&rewrite_active)) {
    LOGW("AOF rewrite already in progress");
    return;
  }
  ...
  LOGI("AOF rewrite started in background");
}

/* ─── AOF_shutdown function ─── */
void AOF_shutdown(void) {
  LOGI("AOF shutdown initiated...");
  ...
  if (writer) {
    ...
    LOGI("Writer thread joined");
  }

  if (atomic_load(&g_uring_ctx.initialized)) {
    pthread_join(uring_worker, NULL);
    LOGI("io_uring worker thread joined");
  }

  /* Wait for any active rewrite to complete */
  if (atomic_load(&rewrite_active)) {
    LOGI("Waiting for rewrite to complete...");
...
/* ─── Status functions ─── */
int AOF_rewrite_in_progress(void) { return atomic_load(&rewrite_active); }
...
size_t AOF_pending_writes(void) {
  ...
  return pending + (atomic_load(&g_uring_ctx.initialized) ? atomic_load(&g_uring_ctx.pending_writes) : 0);
}

void AOF_prepare_for_rotation(void) {
  LOGI("Preparing AOF for rotation...");
...
int AOF_rotate_file(const char *new_path) {
  ...
  LOGI("Draining %d pending io_uring operations...", atomic_load(&g_uring_ctx.pending_writes));

  int timeout_ms = 5000; /* 5 second timeout */
  while (atomic_load(&g_uring_ctx.pending_writes) > 0 && timeout_ms > 0) {
    struct io_uring_cqe *cqe;
    int ret = io_uring_wait_cqe_timeout(&g_uring_ctx.ring, &cqe, &(struct __kernel_timespec){0, 10000000}); /* 10ms */
    if (ret == 0) {
      write_batch_t *batch = (write_batch_t *)cqe->user_data;
      if (batch) {
        free(batch->buffer);
  ...
  if (atomic_load(&g_uring_ctx.pending_writes) > 0) {
    LOGW("Timeout waiting for pending writes, forcing rotation");
  }
  ...
  LOGI("New AOF file opened: %s (fd=%d, generation=%" PRIu64 ")", g_fd_state.file_path, new_master_fd, new_gen);

  /* Release write lock - workers can now acquire new fds */
  pthread_rwlock_unlock(&g_fd_state.rotation_rwlock);

  LOGI("Safe AOF rotation complete");
  return 0;
}

static int aof_tail_reopen(const char *path, int *fd_tail, struct stat *st_out, ino_t *cur_ino, off_t *off,
                           size_t *have) {
  if (*fd_tail >= 0)
    close(*fd_tail);
...
static void *aof_live_tail_loop(void *arg) {
  ...
  if (aof_tail_reopen(path, &fd_tail, &st, &cur_ino, &off, &have) != 0) {
    while (!atomic_load(&g_live_tail_stop) && aof_tail_reopen(path, &fd_tail, &st, &cur_ino, &off, &have) != 0) {
      usleep(2000);
    }
  }
...
void AOF_live_follow_stop(void) {
  if (!atomic_load(&g_live_tail_started))
    return;
