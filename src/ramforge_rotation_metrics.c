...
static void cleanup_old_files_by_count(const char *pattern, int keep_count) {
  ...
  for (int i = keep_count; i < (int)glob_result.gl_pathc; i++) {
    ...
    LOGI("Removing old file: %s (%zu MB)", glob_result.gl_pathv[i], size_mb);
...
static RotationReason should_rotate_rdb(void) {
  ...
  if (size_mb > g_policy.max_rdb_size_mb) {
    LOGI("RDB rotation triggered by size: %zu MB > %zu MB", size_mb, g_policy.max_rdb_size_mb);
    return ROTATION_SIZE;
  }
  time_t age_hours = get_file_age_hours(current_rdb);
  if (age_hours > g_policy.max_age_hours) {
    LOGI("RDB rotation triggered by age: %ld hours > %ld hours", age_hours, g_policy.max_age_hours);
    return ROTATION_AGE;
  }
  ...
  if (snapshots > 0) {
    ...
    if (avg_time_ms > g_policy.max_snapshot_time_ms) {
      LOGI("RDB rotation triggered by performance: %.2f ms > %.2f ms", avg_time_ms, g_policy.max_snapshot_time_ms);
  ...
  if (g_policy.chaos_rotation_enabled) {
    ...
    if (chaos_roll < g_policy.chaos_rotation_chance_pct) {
      LOGI("RDB rotation triggered by chaos: %d%% chance hit!", g_policy.chaos_rotation_chance_pct);
...
static void execute_rdb_rotation(RotationReason reason) {
  ...
  if (rename(old_path, new_path) == 0) {
    LOGI("RDB rotated: %s → %s", old_path, new_path);
    ...
    LOGE("RDB rotation failed: %s", strerror(errno));
    atomic_fetch_add(&g_metrics.rotation_failures, 1);
  }
  clock_gettime(CLOCK_MONOTONIC, &end);
  uint64_t rotation_time_us = (end.tv_sec - start.tv_sec) * 1000000 + (end.tv_nsec - start.tv_nsec) / 1000;
  LOGI("RDB rotation completed in %" PRIu64 " μs", rotation_time_us);
...
static RotationReason should_rotate_aof(void) {
  ...
  if (size_mb > g_policy.max_aof_size_mb) {
    LOGI("AOF rotation triggered by size: %zu MB > %zu MB", size_mb, g_policy.max_aof_size_mb);
    return ROTATION_SIZE;
  }

  time_t age_hours = (time(NULL) - st.st_mtime) / 3600;
  if (age_hours > g_policy.max_age_hours) {
    LOGI("AOF rotation triggered by age: %ld hours > %ld hours", age_hours, g_policy.max_age_hours);
    return ROTATION_AGE;
  }
  ...
  if (pending > 10000) { /* Very high pending writes */
    LOGW("AOF rotation triggered by high pending writes: %" PRIu64, pending);
    return ROTATION_PERFORMANCE;
  }
...
static void execute_aof_rotation(RotationReason reason) {
  ...
  LOGI("Starting enhanced AOF rotation (reason: %s)", reason == ROTATION_SIZE  ? "size"
                                                      : reason == ROTATION_AGE ? "age"
                                                                               : "forced");
  ...
  if (current_size <= 0) {
    LOGW("Skipping rotation of empty/missing AOF file");
    pthread_mutex_unlock(&g_rotation_lock);
    return;
  }

  if (current_size < 1024) { /* Less than 1KB */
    LOGW("Skipping rotation of tiny AOF file (%lld bytes)", (long long)current_size);
    pthread_mutex_unlock(&g_rotation_lock);
    return;
  }
  ...
  LOGI("AOF rotated: %s → %s (size=%lld bytes)", g_aof_base_path, ts_path, (long long)final_size);

  /* Start background rewrite if file is substantial */
  if (final_size > 1024) { /* Only rewrite files > 1KB */
    if (AOF_begin_rewrite(ts_path) == 0) {
      LOGI("AOF segment rewrite started on %s", ts_path);
    } else {
      LOGW("AOF segment rewrite skipped (already running)");
    }
  } else {
    LOGW("Skipping rewrite for small segment %s (%lld bytes)", ts_path, (long long)final_size);
  }
  ...
  uint64_t rotation_time_us =
      (rotation_end.tv_sec - rotation_start.tv_sec) * 1000000 + (rotation_end.tv_nsec - rotation_start.tv_nsec) / 1000;

  LOGI("AOF rotation completed in %" PRIu64 " μs", rotation_time_us);

  pthread_mutex_unlock(&g_rotation_lock);
  LOGI("Enhanced AOF rotation complete");
}
...
static void enhanced_metrics_timer_cb(uv_timer_t *timer) {
  ...
  if (aof_reason != ROTATION_NONE) {
    ...
    if (access(g_aof_base_path, F_OK) == 0) {
      ...
      LOGW("AOF file not found, skipping rotation: %s", g_aof_base_path);
    }
  }
  ...
  if (now - last_check_time > 300) { /* Every 5 minutes */
    ...
    if (pending > 1000) { /* Arbitrary threshold */
      LOGW("High pending write count: %" PRIu64, pending);
    }

    /* Log rotation health */
    uint64_t rotation_failures = atomic_load(&g_metrics.rotation_failures);
    uint64_t total_rotations = atomic_load(&g_metrics.aof_rotations_total) + atomic_load(&g_metrics.snapshots_rotated);

    if (total_rotations > 0) {
      double failure_rate = (double)rotation_failures / total_rotations * 100.0;
      if (failure_rate > 5.0) { /* More than 5% failure rate */
        LOGW("High rotation failure rate: %.1f%% (%" PRIu64 "/%" PRIu64 ")", failure_rate, rotation_failures,
             total_rotations);
      }
    }
  }
...
void RAMForge_rotation_init(const char *rdb_base_path, const char *aof_base_path) {
  ...
  if (AOF_rotate_file_safe) {
    LOGI("Safe AOF rotation detected and enabled");
  } else {
    LOGI("Using standard AOF rotation (safe rotation not available)");
  }
  ...
  LOGI("Enhanced RAMForge Auto-Rotation + Metrics initialized");
  LOGI("RDB Max Size: %zu MB | Max Age: %ld hours | Keep: %d files", g_policy.max_rdb_size_mb, g_policy.max_age_hours,
       g_policy.max_snapshots_to_keep);
  LOGI("AOF Max Size: %zu MB | Max Age: %ld hours | Keep: %d segments", g_policy.max_aof_size_mb,
       g_policy.max_age_hours, g_policy.max_aof_segments_to_keep);
}

void RAMForge_configure_rotation_policy(size_t max_rdb_mb, time_t max_age_hours, int keep_count, int chaos_enabled) {
  ...
  LOGI("Rotation policy updated: Size=%zuMB, Age=%ldhrs, Keep=%d, Chaos=%s", max_rdb_mb, max_age_hours, keep_count,
       chaos_enabled ? "ON" : "OFF");
}

void RAMForge_force_rotation(void) {
  LOGI("Forcing RDB rotation...");
  LOGI("Forcing AOF rotation...");
...
static void update_rotation_scheduler(uint64_t latency_us) {
  ...
  if (!g_is_rotation_manager)
    return; /* workers exit early */

  if (g_scheduler.consecutive_slow_operations > 100 && g_scheduler.performance_score < 0.95) { // Less than 95% sub-1ms
    LOGI("Performance-based rotation triggered: %.1f%% sub-1ms performance", g_scheduler.performance_score * 100);
...
void RAMForge_enhanced_operation_record(uint64_t latency_us) { update_rotation_scheduler(latency_us); }
...
void RAMForge_cleanup_rotation_system(void) {
  ...
  LOGI("RAMForge rotation system cleaned up");
}
