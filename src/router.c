...
static void join_segments(char *dst, size_t dst_cap, char *segments[], int i, int j) {
    ...
    for (int k = i; k <= j; k++) {
        ...
        if (pos && pos < dst_cap-1) dst[pos++] = '/';
        if (pos + sl >= dst_cap) {
            size_t room = (dst_cap > pos) ? dst_cap - pos - 1 : 0;
            if (room) { memcpy(dst + pos, segments[k], room); pos += room; }
            break;
        }
        memcpy(dst + pos, segments[k], sl); pos += sl;
    }
    if (dst_cap) dst[(pos < dst_cap)?pos:(dst_cap-1)] = '\0';
}
                      ...
                      char *response_buffer)
{
    ...
    for (int i = 0; i < seg_count; i++) {
        ...
        while (cur) {
            ...
            if (cur->is_param) {
                if (cur->is_splat) splat_match = cur; else param_match = cur;
            }
            cur = cur->sibling;
        }
            ...
            if (splat_match) {
                ...
                if (!splat_match->children) {
                    ...
                    if (req.param_count < MAX_ROUTE_PARAMS) {
                        ...
                        join_segments(req.params[req.param_count].value,
                                      sizeof(req.params[0].value),
                                      segments, i, seg_count-1);
                        req.param_count++;
                    }
...
static void free_trie(TrieNode *n) {
    while (n) {
        ...
        if (n->children) free_trie(n->children);
        free(n->segment);
        free(n->param_name);
        free(n);
        n = next;
