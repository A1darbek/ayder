...
typedef enum { JSON_NULL, JSON_BOOL, JSON_INT, JSON_DOUBLE, JSON_STRING, JSON_OBJECT, JSON_ARRAY } json_type_t;
...
static int parse_value(json_parser_t *p, json_value_t *out) {
  ...
  if (c == '"') {
    ...
    while (p->ptr < p->end && (((*p->ptr >= '0') && (*p->ptr <= '9')) || (*p->ptr == '.') || (*p->ptr == 'e') ||
                               (*p->ptr == 'E') || (*p->ptr == '+') || (*p->ptr == '-'))) {
      p->ptr++;
    }
...
static inline json_value_t *json_parse(const char *input, size_t len) {
  json_parser_t parser = {.input = input, .ptr = input, .end = input + len, .error_pos = 0};
...
static inline void json_deinit(json_value_t *val) {
  ...
  default: /* JSON_STRING/INT/BOOL/NULL: no owning heap */
    break;
  }
}
...
static const json_template_t USER_TEMPLATE = {.template      = "{\"id\":,\"name\":\"\"}",
                                              .template_len  = 17,
